...
def predict_ANN(target = str, x_test = np.array):
    ...
    model.load_state_dict(torch.load("prediction\\model_all_"+target+".pth"))
    model.eval()
    ...
    scaler_read = open('prediction\\'+target+'_scaler.txt','r')
...
def predict_RF(target = str, x_test= np.array):
    ...
    with open('prediction\\RF_'+target+'.pickle','rb') as f:
        rf = pickle.load(f)
...
read = pd.read_csv("prediction\\Tc_T.csv",usecols = ['name','Tc','Tb'])
#调取基础数据，沸点和临界温度。
#Get the data from the database, boiling point and critical temperature.

print('This is a binary VLE prediction model,please input the first component')
name1 = input()
...
x_scaler_read = open('prediction\\input_scaler.txt','r')
x_min = []
x_scaler = []
